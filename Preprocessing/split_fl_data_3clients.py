...
while count < client_num :
    np.save('x_bal-train-client ' + str(count + 1), x_train[count])
    np.save('y_bal-train-client ' + str(count + 1), y_train[count])
    
    np.save('x_bal-test-client ' + str(count + 1), x_test[count])
    np.save('y_bal-test-client ' + str(count + 1), y_test[count])
    count = count + 1