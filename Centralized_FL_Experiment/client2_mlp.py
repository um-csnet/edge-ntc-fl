...
#load dataset
x_train = np.load("x_bal-train-client 2.npy")
y_train = np.load("y_bal-train-client 2.npy")

x_test = np.load("x_bal-test-client 2.npy")
y_test = np.load("y_bal-test-client 2.npy")

#load dataset
#x_train = np.load("x_bal-twotrain-client 2.npy")
#y_train = np.load("y_bal-twotrain-client 2.npy")

#x_test = np.load("x_bal-twotest-client 2.npy")
#y_test = np.load("y_bal-twotest-client 2.npy")


#x_train = np.load("x_twotrain-client 2.npy")
#y_train = np.load("y_flip_twotrain-client 2.npy")
...
display_name = "fl_client2_mlp_3client_36epochs_1round_gpu"
...
class ntcClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        #history = model.fit(x_train, y_train, epochs=36, batch_size=64, shuffle = True, callbacks=[WandbMetricsLogger(log_freq=5),WandbModelCheckpoint("models")])
        return model.get_weights(), len(x_train), {'train_loss':history.history['loss'][0]}

    def evaluate(self, parameters, config):
        model.set_weights(parameters)
        loss, accuracy = model.evaluate(x_test, y_test)
        model.save("model_client2_mlp_3client_36epochs_1round_gpu.h5")
